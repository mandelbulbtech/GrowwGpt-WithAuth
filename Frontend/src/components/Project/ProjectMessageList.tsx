...
const ProjectMessageList: React.FC<ProjectMessageListProps> = ({ messages }) => {
  const endOfMessagesRef = useRef<HTMLDivElement>(null);
  const { loadingMessage, currentProject } = useConversation();
  const [processedMessages, setProcessedMessages] = useState<Message[]>([]);

  // Process messages to ensure document_names persistence
  useEffect(() => {
    const enrichedMessages = messages.map(message => {
      // Only process user messages without document_names
      if (message.role === 'user' && (!message.document_names || message.document_names.length === 0)) {
        // Create a unique key based on message content and timestamp
        const messageKey = `file_names_${message.content}_${message.timestamp}`;
        const fileNames = localStorage.getItem(messageKey);

        if (fileNames) {
          return {
            ...message,
            document_names: JSON.parse(fileNames),
            role: message.role as 'user' | 'assistant' // Ensure correct typing
          };
        }

        // Fall back to old storage method for backward compatibility
        const oldFileNames = localStorage.getItem('file_names_' + message.content);
        if (oldFileNames) {
          // Also save to new format for future use
          localStorage.setItem(messageKey, oldFileNames);
          return {
            ...message,
            document_names: JSON.parse(oldFileNames),
            role: message.role as 'user' | 'assistant' // Ensure correct typing
          };
        }
      }
          ...
          <div className={`flex items-start ${message.role === 'user' ? 'flex-row-reverse justify-end' : 'flex-row justify-start'}`}>
            ...
            <div className={`flex flex-col ${message.role === 'user' ? 'w-[100%]' : 'w-fit'}`}>
              ...
              <div className={`rounded-lg px-3 py-2 ${message.role === 'user'
                ...
                {/* If there are document attachments, show them */}
                ...
                {/* Show documents processed badge for assistant responses */}
                {message.role === 'assistant' && message.documents_processed && message.documents_processed > 0 && (
                  <div className="mb-2">
                    <span className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-md bg-blue-50 text-blue-700">
                      ...
                      {message.documents_processed} document{message.documents_processed > 1 ? 's' : ''} processed
                    </span>
                  </div>
                )}
            ...
            <div className="flex flex-col">
              <div className="bg-gray-100 text-groww-black rounded-lg px-3 py-2 w-fit">
                <div className="flex items-center space-x-2">
                  <div className="animate-pulse">Processing project data</div>
